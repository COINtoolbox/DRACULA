...
#in_file = "../data_all_types/derivatives_train.dat" ** this should be input ***
in_file = "../../../data_all_types/derivatives.dat"

run_dir='./'

# layers
input_string = '120,100,90,50,30,20,2,20,30,50,90,100,120'  #** this should be input ***
# the output layer is the 7th, the central one
n_layers = 7                  #** this should be input ***
# random seed
seed = 1                      #** this should be input, optional default is seed=1 ***
...
r('train2.hex <- h2o.importFile(path = TRAIN, header = F, parse=TRUE, col.names=NULL, col.types=NULL, sep = ",", destination_frame="train2.hex")')
...
# before saving the output file, check if directory exists, if not, create it
np.savetxt(run_dir+'out_DeepLearning/out_%s_seed%d_dl.dat' % (input_string,seed)  ,X) #name of output files should be input, optional



# this is the test sample 
in_file = "../../../data_all_types/derivatives.dat"   # ** this should be input, optional, default should be equal to line 6 ***
...
r('test.hex <- h2o.importFile(path = TEST, header = F, parse=TRUE, col.names=NULL, col.types=NULL, sep = ",", destination_frame="test.hex")')
...
np.savetxt(run_dir+'out_DeepLearning/predictions_%s_seed%d_dl.dat' % (input_string,seed) ,pred) #name of output files should be input, optional

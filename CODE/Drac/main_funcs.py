...
class set_prop:
	def __init__(self): 
		...
		self.dict_red	= set_red_dict()
		self.dict_clust	= set_cl_dict()

		self.MASK			= set_mask()
		self.RED_DATA,self.RED_TYPE	= set_red(self.dict_red)
		self.CL_DATA,self.CL_TYPE	= set_cl(self.dict_clust)
		self.LAB_DATA			= set_lab(self.dict_clust)
...
def reducC(CONFIG,case=''):
	...
	try		: ORG_DATA
		...
		print_info(REDUCTION_METHOD,CONFIG.dict_red,'### REDUCTION USED ###',ORG_DATA,CONFIG.REDUCTION_INFO+case)
...
def clusterC(CONFIG,case_in='',case_out=''):
	...
	print_info(CLUSTERING_METHOD,CONFIG.dict_clust,'### CLUSTERING USED ###',CONFIG.RED_DATA,CONFIG.CLUSTER_INFO+case_out,'a')
